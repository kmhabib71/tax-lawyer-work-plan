/**
 * Final Bengali Legal Text Cross-Reference Parser
 * Correctly handles Bengali Unicode characters and legal text patterns
 * Based on detailed character analysis and real document testing
 */

class FinalBengaliParser {
    constructor() {
        this.crossReferencePatterns = this.initializeFinalPatterns();
        this.documentMap = new Map();
        this.extractedReferences = [];
    }

    /**
     * Final regex patterns with correct Bengali Unicode character handling
     */
    initializeFinalPatterns() {
        return {
            // Act references - Corrected with proper Bengali Unicode
            act_references: [
                // Pattern 1: আয়কর আইন, ২০২৩ (২০২৩ সনের ১২ নং আইন) এর ২য় ধারায়
                /(আয়কর আইন|অর্থ আইন|কোম্পানি আইন|প্রতিবন্ধী ব্যক্তির অধিকার ও সুরক্ষা আইন),\s*([০-৯]{4})\s*\(([০-৯]{4})\s*সনের\s*([০-৯]+)\s*নং আইন\)\s*এর\s*([০-৯]+)(?:য়|ম|তম)?\s*ধারা(?:য়|র|তে)?(?:\s*(?:এর\s*)?উপ-ধারা\s*\(([০-৯]+)\))?/g,
                
                // Pattern 2: আয়কর আইন, ২০২৩ এর ২য় ধারার উপ-ধারা (১)
                /(আয়কর আইন|অর্থ আইন|কোম্পানি আইন|প্রতিবন্ধী ব্যক্তির অধিকার ও সুরক্ষা আইন),\s*([০-৯]{4})\s*এর\s*([০-৯]+)(?:য়|ম|তম)?\s*ধারা(?:য়|র|তে)?(?:\s*(?:এর\s*)?উপ-ধারা\s*\(([০-৯]+)\))?/g,
                
                // Pattern 3: উক্ত আইনের ধারা ৪ এর উপ-ধারা (১)
                /উক্ত আইনে?র?\s*(?:ধারা\s*)?([০-৯]+)\s*(?:তম|য়|ম)?\s*ধারা(?:য়|র|তে)?(?:\s*(?:এর\s*)?উপ-ধারা\s*\(([০-৯]+)\))?/g,
                
                // Pattern 4: এই আইনের ৮৫তম ধারা
                /এই আইনে?র?\s*(?:ধারা\s*)?([০-৯]+)\s*(?:তম|য়|ম)?\s*ধারা(?:য়|র|তে)?(?:\s*(?:এর\s*)?উপ-ধারা\s*\(([০-৯]+)\))?/g,

                // Pattern 5: কোম্পানি আইন, ১৯৯ৄ এর অধীন
                /(কোম্পানি আইন|আয়কর আইন|অর্থ আইন),\s*([০-৯]{4})\s*(?:\([^)]*\))?\s*এর\s*অধীন/g
            ],

            // Rules and regulations - Corrected
            rules_references: [
                // Pattern 1: উৎসে কর কর্তন বিধিমালা, ২০২৪ এর ৩য় ও ৄর্থ নিয়ম
                /(উৎসে কর কর্তন বিধিমালা|ভ্যাট ও সম্পূরক শুল্ক বিধিমালা|[^,]*বিধিমালা),\s*([০-৯]{4})\s*এর\s*([০-৯]+)(?:য়|র্থ|তম)?\s*(?:ও\s*([০-৯]+)(?:র্থ|তম)?\s*)?নিয়ম/g,
                
                // Pattern 2: উক্ত বিধিমালার ৩য় নিয়ম
                /উক্ত বিধিমালার?\s*([০-৯]+)(?:য়|র্থ|তম)?\s*(?:ও\s*([০-৯]+)(?:র্থ|তম)?\s*)?নিয়ম/g
            ],

            // SRO notifications - Corrected  
            sro_references: [
                // Pattern 1: এসআরও নং ২৪১/২০২৪
                /এসআরও\s*নং?\s*([০-৯]{2,3}\/[০-৯]{4})/g,
                
                // Pattern 2: এসআরও নং ২৪১/২০২৪ এর ২য় ধারা
                /এসআরও\s*নং?\s*([০-৯]{2,3}\/[০-৯]{4})\s*এর?\s*([০-৯]+)(?:তম|য়|ম)?\s*ধারা/g
            ],

            // NBR circulars and orders - Corrected
            circular_references: [
                // Pattern 1: জাতীয় রাজস্ব বোর্ডের পরিপত্র নং ০৯/২০২৪
                /(?:জাতীয় রাজস্ব বোর্ডের\s*)?পরিপত্র\s*নং?\s*([০-৯]{1,2}\/[০-৯]{4})/g,
                
                // Pattern 2: জাতীয় রাজস্ব বোর্ডের সাধারণ আদেশ নং ১৫/২০২৪
                /(?:জাতীয় রাজস্ব বোর্ডের\s*)?সাধারণ আদেশ\s*নং?\s*([০-৯]{1,2}\/[০-৯]{4})/g
            ],

            // Schedule references - Corrected
            schedule_references: [
                // Pattern 1: প্রথম তফসিলের ৩য় অংশ
                /(প্রথম|দ্বিতীয়|তৃতীয়|চতুর্থ|পঞ্চম|ষষ্ঠ|সপ্তম|অষ্টম)\s*তফসিল(?:ে|ের|এর)?(?:\s*([০-৯]+)(?:য়|ম|র্থ)\s*অংশ)?/g,
            ...
            // Ordinance references - Corrected
            ordinance_references: [
                // Pattern 1: আয়কর অধ্যাদেশ, ১৯৮৪ এর ২৮৫ ধারা
                /(আয়কর অধ্যাদেশ|[^,]*অধ্যাদেশ),\s*([০-৯]{4})\s*এর?\s*([০-৯]+)\s*ধারা(?:য়|র)?/g
            ]
        };
    }
    ...
    parseDocument(documentContent, documentPath) {
        ...
        // Extract different types of references with corrected patterns
    ...
    /**
     * Enhanced Act references extraction with corrected patterns
     */
    extractActReferences(text, sourcePath) {
        const references = [];
        
        this.crossReferencePatterns.act_references.forEach((pattern, patternIndex) => {
            let match;
            while ((match = pattern.exec(text)) !== null) {
                ...
                if (patternIndex <= 1) {
                    ...
                    reference = {
                        ...
                        target_section: 'general_authority'
                    };
                }
    ...
    // Helper methods (same as before with minor improvements)
    ...
    calculateConfidence(matchText, type) {
        ...
        if (matchText.includes('ধারা') && /[০-৯]/.test(matchText)) confidence += 0.2;
        if (matchText.includes('উপ-ধারা')) confidence += 0.1;
        if (matchText.includes('নং')) confidence += 0.1;
        if (/[০-৯]{4}/.test(matchText)) confidence += 0.1; // Year present
    ...
    resolveContextualTarget(sourcePath) {
        if (sourcePath.includes('finance_act')) {
            return '01_income_tax_act_simulation.json';
        }
        return sourcePath;
    }
    ...
    analyzeCoverage() {
        ...
        this.documentMap.forEach((docData, docPath) => {
            ...
            coverage.referenceDensity[docPath] = {
                ...
                density: docData.references.length / (docData.text.length / 1000)
            };
        });
...
module.exports = FinalBengaliParser;