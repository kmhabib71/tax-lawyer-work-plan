/**
 * Improved Bengali Legal Text Cross-Reference Parser
 * Enhanced to better detect natural Bengali legal cross-references
 */

class ImprovedBengaliParser {
    constructor() {
        this.crossReferencePatterns = this.initializeImprovedPatterns();
        this.documentMap = new Map();
        this.extractedReferences = [];
    }

    /**
     * Enhanced regex patterns for Bengali legal cross-references
     */
    initializeImprovedPatterns() {
        return {
            // Act references - more flexible patterns
            act_references: [
                // Pattern: আয়কর আইন, ২০২৩ এর ৮৫তম ধারা
                /([আয়কর আইন|অর্থ আইন|মূল্য সংযোজন কর ও সম্পূরক শুল্ক আইন|কাস্টমস আইন|ভ্রমণ কর আইন]),?\s*([১২]\d{3})\s*(?:\([^)]*\))?\s*এর\s*(?:ধারা\s*)?(\d+(?:[ক-ৎ])?)\s*(?:তম\s*)?ধারা(?:য়)?(?:\s*এর\s*উপ-ধারা\s*\((\d+)\))?/g,
                
                // Pattern: আয়কর আইন, ২০২৩ (২০২৩ সনের ১২ নং আইন) এর ৮৫তম ধারা
                /([আয়কর আইন|অর্থ আইন|মূল্য সংযোজন কর ও সম্পূরক শুল্ক আইন|কাস্টমস আইন|ভ্রমণ কর আইন]),?\s*([১২]\d{3})\s*\(([১২]\d{3})\s*সনের\s*(\d+)\s*নং আইন\)\s*এর\s*(?:ধারা\s*)?(\d+(?:[ক-ৎ])?)\s*(?:তম\s*)?ধারা(?:য়)?(?:\s*এর\s*উপ-ধারা\s*\((\d+)\))?/g,
                
                // Pattern: উক্ত আইনের ৮৫তম ধারা
                /উক্ত আইনে?র?\s*(?:ধারা\s*)?(\d+(?:[ক-ৎ])?)\s*(?:তম\s*)?ধারা(?:য়)?(?:\s*এর\s*উপ-ধারা\s*\((\d+)\))?/g,
                
                // Pattern: এই আইনের ৮৫তম ধারা  
                /এই আইনে?র?\s*(?:ধারা\s*)?(\d+(?:[ক-ৎ])?)\s*(?:তম\s*)?ধারা(?:য়)?(?:\s*এর\s*উপ-ধারা\s*\((\d+)\))?/g
            ],

            // Rules and regulations - enhanced
            rules_references: [
                // Pattern: উৎসে কর কর্তন বিধিমালা, ২০২৪ এর ৩য় নিয়ম
                /(উৎসে কর কর্তন বিধিমালা|ভ্যাট ও সম্পূরক শুল্ক বিধিমালা|[^,]*বিধিমালা),?\s*([১২]\d{3})\s*এর\s*(\d+(?:[ক-ৎ])?)\s*(?:য়)?\s*নিয়ম/g,
                
                // Pattern: উক্ত বিধিমালার ৩য় নিয়ম
                /উক্ত বিধিমালার?\s*(\d+(?:[ক-ৎ])?)\s*(?:য়)?\s*নিয়ম/g,
                
                // Pattern: এই বিধিমালার ৩য় নিয়ম
                /এই বিধিমালার?\s*(\d+(?:[ক-ৎ])?)\s*(?:য়)?\s*নিয়ম/g
            ],

            // SRO notifications - enhanced  
            sro_references: [
                /এসআরও\s*নং?\s*([২৩]\d{2}\/[১২]\d{3})/g,
                /(?:এস\.আর\.ও|S\.R\.O\.?)\s*(?:নং?)?\s*([২৩]\d{2}\/[১২]\d{3})/g
            ],

            // NBR circulars and orders - enhanced
            circular_references: [
                /(?:জাতীয় রাজস্ব বোর্ডের\s*)?পরিপত্র\s*নং?\s*(\d{1,2}\/[১২]\d{3})/g,
                /(?:জাতীয় রাজস্ব বোর্ডের\s*)?সাধারণ আদেশ\s*নং?\s*(\d{1,2}\/[১২]\d{3})/g
            ],

            // Schedule references - enhanced
            schedule_references: [
                ...
                /এই আইনের\s*(প্রথম|দ্বিতীয়|তৃতীয়|চতুর্থ|পঞ্চম|ষষ্ঠ|সপ্তম|অষ্টম)\s*তফসিল/g
            ],

            // Ordinance references - enhanced
            ordinance_references: [
                /(আয়কর অধ্যাদেশ|[^,]*অধ্যাদেশ),?\s*([১২]\d{3})\s*এর\s*(?:ধারা\s*)?(\d+(?:[ক-ৎ])?)\s*(?:তম\s*)?ধারা(?:য়)?/g
            ]
        };
    }
    ...
    parseDocument(documentContent, documentPath) {
        ...
        // Extract different types of references with improved patterns
    ...
    /**
     * Enhanced Act references extraction
     */
            ...
            while ((match = pattern.exec(text)) !== null) {
                ...
                if (match[1]) {
                    ...
                    reference = {
                        ...
                        year: match[2],
                        act_number: match[4],
                        section: match[5] || match[3],
                        subsection: match[6] || match[4],
                        ...
                        target_section: this.formatSectionReference(match[5] || match[3], match[6] || match[4])
                    };
                } else {
...
module.exports = ImprovedBengaliParser;