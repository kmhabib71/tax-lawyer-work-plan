    ...
    def extract_sections_from_text(self, text_file_path, output_file_path):
        ...
        for pattern in section_patterns:
            ...
            for match in matches:
                ...
                # Get actual content after the section heading
                start_pos = match.end()
                # Look for next section or end of content to get full section
                next_section_match = None
                for next_pattern in section_patterns:
                    next_match = re.search(next_pattern, content[start_pos:start_pos+3000])
                ...
                if next_section_match:
                    section_content = content[start_pos:start_pos+next_section_match.start()].strip()
                else:
                    section_content = content[start_pos:start_pos+1500].strip()
                
                # Clean up the content
                section_content = re.sub(r'\n\s*\n\s*\n', '\n\n', section_content)  # Remove excessive line breaks
                section_content = re.sub(r'### PAGE \d+ ###', '', section_content)  # Remove page markers
                ...
                extracted_sections.append({
                    ...
                    "type": "section",
                    "has_subsections": "(" in section_content and ")" in section_content
                })
